...
uses Windows, SysUtils,      //    www.jps.net/gfody
     FastRGB,      //
...
// Fixed for 64 bit by Charles Hollemeersch 2021
...
begin
  ...
  Gap:=RowInc-Width;
  Size:=RowInc*Height;
  ReallocMem(Pixels,Height*sizeof(PtrUInt));
  x:=PtrUint(Bits);
...
var
hFile:  Integer;
{$IFDEF VER120} iWrote: Cardinal;
{$ELSE}         iWrote: LongWord;
{$ENDIF}
fData:  TofStruct;
fHead:  TBitmapFileHeader;
begin
...
begin
  ...
  begin
    ...
    pc:=Pointer(PtrUint(pc)+FB.Gap); { *Converted from Ptr* }
    pb:=Pointer(PtrUint(pb)+Gap); { *Converted from Ptr* }
  end;
  Dispose(Space);
end;
...
begin
  ...
  begin
    ...
    Tmp:=Pointer(PtrUint(Tmp)+FB.Gap); { *Converted from Ptr* }
    p:=Pointer(PtrUint(p)+Gap); { *Converted from Ptr* }
  end;
  for i:=0 to 255 do
  begin
...
begin
  ...
  GetMem(Buf,Amount*4);
  CopyMemory(Buf,Pointer(PtrUint(@bmInfo.bmiColors)+((256-Amount)*4)) { *Converted from Ptr* },Amount*4);
  MoveMemory(Pointer(PtrUint(@bmInfo.bmiColors)+(Amount*4)) { *Converted from Ptr* },@bmInfo.bmiColors,(256-Amount)*4);
...
begin
  ...
  begin
    CopyMemory(n1,n2,w);
    n1:=Pointer(PtrUint(n1)+Dst.RowInc); { *Converted from Ptr* }
    n2:=Pointer(PtrUint(n2)+RowInc); { *Converted from Ptr* }
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      pc^:=
        (Col1^*w1 +
         (PByte(Pointer(PtrUint(Col1)+1))^)*w2 +
         Col2^*w3 +
         (PByte(Pointer(PtrUint(Col2)+1))^)*w4 )shr 15;
      Inc(pc);
      Inc(xP,xP2);
    end;
    Inc(yP,yP2);
